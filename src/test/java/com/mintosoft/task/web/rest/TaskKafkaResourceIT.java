...
import com.mintosoft.task.config.KafkaProperties;
...
class TaskKafkaResourceIT {
    ...
    void setup() {
        ...
        TaskKafkaResource kafkaResource = new TaskKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/task-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/task-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
